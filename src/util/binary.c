...
uint32_t remove_padding(char *data, size_t data_len, size_t significant_bits_per_char)
{
    if (significant_bits_per_char <= 0)
        return 0;
...
void read_int(uint32_t *target, FILE *origin, bool int_endianess, bool cpu_endianess)
{
...
{
    read_int(target, origin, true, system_is_little_endian());
}

void read_big_endian_int(uint32_t *target, FILE *origin)
{
    read_int(target, origin, true, system_is_big_endian());
}
